...
var Sakura = function Sakura(selector, options) {
  ...
  this.createPetal = function () {
    ...
    petal.classList.add(_this.settings.className);
     // Add the top position line here so that each petal starts at the top
    petal.style.top = "-50px";  // This line ensures the petal starts from above the viewport

     // Then, you can set the left position as you have it in your current code
    ...
    //petal.style.left = "".concat(Math.random() * document.documentElement.clientWidth - 100, "px"); Removed so is not duplicated
